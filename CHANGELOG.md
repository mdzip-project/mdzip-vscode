...
- **Convert-to-.mdz is now one checkbox prompt** instead of a chain of modal dialogs, with new options: add a human-facing `README.md`, add an agent-facing `AGENTS.md` (issue #10), delete the original `.md` afterward (to the OS trash), name the output `<file>.mdz.zip` for recipients without MDZip, and remember the selections as defaults.
- **`.mdz.zip` support.** Files named `<file>.mdz.zip` open in the MDZip editor, and every `.mdz` command (Add AGENTS.md, Extract to Folder, Git compare) accepts them, via a shared `isMdzUri()` check.
- **Claude Code MCP setup.** New `mdzip.enableClaudeMcp` and `mdzip.copyClaudeMcpConfigSnippet` write/merge the workspace `.mcp.json` (`mcpServers` schema) through the version-independent launcher, with self-healing repair of stale version-pinned entries on activation. (issue #8)
- **Agent guidance when no MCP server is present.** "MDZip: Enable Workspace Agent Instructions" now writes a "Working with `.mdz` Files" section covering format identification, the MCP-enable commands, and a no-MCP entry-point-discovery fallback. (issue #4)
- **Document Information dialog** gained Size, Documents, Assets, and a Read-only row (shared `@mdzip/editor` change).
- **Templates can opt out of the default `AGENTS.md`** — `"includeAgents": false` in a folder template's `template.config.json`, or `mdzipIncludeAgents: false` in a single-file template's frontmatter. (issue #3)
- **Off-main-thread archive parsing** in the `.mdz` editor webview (Worker), with a main-thread fallback.

### Fixed
- **Large `.mdz` files no longer crash or hang.** Opening 100 MB+ archives (e.g. chat exports with thousands of embedded images) previously OOM-crashed the extension host or froze the window. Fixed the message-passing OOM (raw `ArrayBuffer` instead of base64 + JSON), enabled progressive preview rendering, and capped raw-byte transfer at 32 MB — larger archives fall back to a full-workspace rebuild for asset mutations instead of crashing.
- **"Convert Folder to .mdz" was ~30× too slow.** A ~30 s stall was traced to `@mdzip/core-js` compressing each file through a fresh `CompressionStream` pipeline in the VS Code webview; replaced with `fflate`'s synchronous `deflateSync` (~30 s → ~0.2 s on an 11-file folder). New dependency: `fflate`.
- **"Convert Folder to .mdz" stuck on "No MDZip workspace loaded"** for folders with more than one `.md` file — an early-return ordering bug in `@mdzip/editor`'s render path (fixed for all three library consumers).
- **Read-only files are now detected.** An OS read-only `.mdz` previously gave no sign until a save failed with a raw `EPERM`; it now opens in read-only mode, is marked in the tab title, and permission-denied saves show an actionable message.
- **Convert-to-.mdz safety.** Dismissing a prompt (Escape) aborts instead of proceeding, and converting no longer silently overwrites an existing `.mdz` without confirmation.
- **Relative images via raw HTML `<img src>`** rendered broken in a plain `.md` opened in the MDZip editor after "Extract to Folder". Fixed at the root by making `@mdzip/core-js`'s `extractImageReferences()` public and routing the editor's and extension's duplicate detectors through it.
...
- #3 Part 1 (insert `AGENTS.md` into an existing archive) and #4 layer 1 (in-archive guidance) — already covered by existing functionality.
